...
3. Follow the project conventions provided by Leader (naming, error handling, response format, architecture patterns). If conventions were not provided, read 2–3 existing files in the same layer before writing any code to infer the pattern.
4. Implement the change or bug fix.
5. Run the test files specified by Leader — verify they were failing before your change, then pass after. If tests cannot be run, state that explicitly.
6. Summarize the result for the leader.
